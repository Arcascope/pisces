...
# %% auto 0
__all__ = ['Base', 'ACCELEROMETER_FOLDER_NAME', 'HEARTRATE_FOLDER_NAME', 'PSG_FOLDER_NAME', 'ACTIGRAPHY_FOLDER_NAME',
           'StudyRecord', 'DatabaseLiaison', 'DataRecord', 'DataPool', 'make_datapool', 'make_database']
...
def make_datapool(config: DatabaseConfiguration, data_folder: str) -> DataPool:
    # Instantiate the tools for database management.
    liaison = (
        make_database(config.data_set_csv, config.database)
        if config.database == ":memory:"
        else DatabaseLiaison(db_filename=config.database)
    )

    records_to_use = liaison.select(config.selector)
...
def make_database(
    data_set_csv_path: str | Path = DEFAULT_DATA_SETS_CSV, db_filename: str = ":memory:"
) -> DatabaseLiaison:

    data_set_csv_path = figure_out_data_sets_path(data_set_csv_path)

    liaison = DatabaseLiaison(db_filename=db_filename)
    with open(data_set_csv_path, newline="") as csvfile:
        rows = list(csv.DictReader(csvfile))

    # Do any manipulation/interpretation of literals here
